...
/** Number-game tier for one entry against drawn numbers. */
function numberTier(entryNumbers, drawn) {
  const hit = entryNumbers.filter(n => drawn.includes(n)).length;
  if (hit === 7) return 'jackpot';
  if (hit === 6) return 'match6';
  if (hit === 5) return 'match5';
  return null;
}

module.exports = {
  CODE_RE, normalizeCode, normalizePhone,
  validateSubmission, scorePicks, rankEntries, computePool, numberTier
};
